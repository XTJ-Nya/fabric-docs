...
试试使用 `gameTimeDeltaPartialTick` 并查看运行 `/tick freeze` 命令时动画会发生什么情况。 你会看到动画在 `gameTimeDeltaPartialTick` 变为常量时冻结（假设你已将 `false` 作为参数传递给 `DeltaTracker#getGameTimeDeltaPartialTick`）
